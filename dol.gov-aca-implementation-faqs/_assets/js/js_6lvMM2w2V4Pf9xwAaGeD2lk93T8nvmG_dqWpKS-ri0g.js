function autoupdate() {
  ...
  {
    type: "GET",
    url: '/api/v1/call-center/wait-time',
    cache: false,
    success: function(data) {
      var result = JSON.parse(data);
      var ds = result['statList'];

      var waitTimes = [];
      waitTimes['wb_spanish'] = ds[0]['Wait'];
      waitTimes['tty_english'] = ds[1]['Wait'];
      waitTimes['osha_english'] = ds[2]['Wait'];
      waitTimes['odep_english'] = ds[3]['Wait'];
      waitTimes['eta_english'] = ds[4]['Wait'];
      waitTimes['vets_english'] = ds[5]['Wait'];
      waitTimes['wb_english'] = ds[6]['Wait'];
      waitTimes['osha_spanish'] = ds[7]['Wait'];
      waitTimes['odep_spanish'] = ds[8]['Wait'];
      waitTimes['osdbu_english'] = ds[9]['Wait'];
      waitTimes['vets4212_english'] = ds[10]['Wait'];
      waitTimes['eta_spanish'] = ds[11]['Wait'];
      waitTimes['msha_spanish'] = ds[12]['Wait'];
      waitTimes['jc_english'] = ds[13]['Wait'];
      waitTimes['jc_spanish'] = ds[14]['Wait'];
      waitTimes['opa_spanish'] = ds[15]['Wait'];
      waitTimes['opa_english'] = ds[16]['Wait'];
      waitTimes['msha_english'] = ds[17]['Wait'];

      jQuery("#doleta").html(waitTimes['eta_english']);
      jQuery('#doljbcr').html(waitTimes['jc_english']);
      jQuery('#dolmsha').html(waitTimes['msha_english']);
      jQuery('#dolosha').html(waitTimes['osha_english']);
      jQuery('#dolodep').html(waitTimes['odep_english']);
      jQuery('#dolosdbu').html(waitTimes['osdbu_english']);
      jQuery('#dolvets100').html(waitTimes['vets_english']);
      jQuery('#dolwb').html(waitTimes['wb_english']);
      jQuery('#doltty').html(waitTimes['tty_english']);
      jQuery('#dolwide').html(waitTimes['opa_english']);

      jQuery('#datetimeRefresh').html(new Date().toLocaleString());
    }
  });
}
